...
# Modify Pre-trained ResNet for 12-band Input
class ResNet12Band(nn.Module):
    def __init__(self, num_classes=10):
        super(ResNet12Band, self).__init__()
        self.resnet = models.resnet18(weights=models.ResNet18_Weights.IMAGENET1K_V1)  # Load pre-trained weights
        self.resnet.conv1 = nn.Conv2d(12, 64, kernel_size=7, stride=2, padding=3, bias=False)  # Modify input layer
        self.resnet.fc = nn.Linear(512, num_classes)  # Modify output layer
...
# Save model
torch.save(model.state_dict(), "eurosat_finetuned_model.pth")
...
# Load and test model
model.load_state_dict(torch.load("eurosat_finetuned_model.pth"))  # Load fine-tuned model
