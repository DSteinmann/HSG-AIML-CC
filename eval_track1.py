...
EVAL_CONFIG = {
    "model": {
        # --- Match the "scratch" training config ---
        "name": "ResNet50_16ch_EuroSAT_Scratch_v1", # Matches scratch trained model name
        "pretrained": False, # Consistent with training config (mainly affects architecture definition)
        "input_channels": 16, # 12 bands + 4 indices
        # !!! Point this to the weights saved by the SCRATCH training script !!!
        "base_save_path": Path('./outputs/resnet50_16ch_eurosat_scratch_v1'), # Base path used in SCRATCH training
        # --- Choose which weights to load ---
        # !!! Update this to the specific checkpoint you want to evaluate !!!
        "load_weights_name": "ResNet50_16ch_EuroSAT_Scratch_v1_best.pth", # Example: Load best model
        # "load_weights_name": "ResNet50_16ch_EuroSAT_Scratch_v1_epoch_XXX.pth", # Example: Load specific epoch
        # ----------------------------------
    ...
    "data": {
        ...
        "image_size": 224, # Should match image_size used during training (224)
    ...
    "prediction": {
        # Output file path reflects the SCRATCH model
        "predictions_csv_path": Path('./outputs/predictions_resnet50_16ch_scratch_v1.csv'),
        "kaggle_competition": 'aicrowd-geospatial-challenge', # Replace if needed
        "kaggle_message": 'Submission ResNet50_16ch_Scratch_v1', # Update message
...
eval_transforms = transforms.Compose([
    ...
    # CenterCrop might be added here if RandomCrop was used successfully in training
    # transforms.CenterCrop((IMG_SIZE, IMG_SIZE)), # Keep commented unless RandomCrop worked
])

# --- Model Definition Function (Using ResNet50 Architecture) ---
# Copied from training script
def adapt_resnet_for_multichannel(model_name="resnet50", pretrained=False, num_classes=10, input_channels=16):
    ...
    # Important: Use pretrained=False here, as we load specific weights later
    print(f"Loading {model_name} architecture definition...")
    model = models.get_model(model_name, weights=None) # Load architecture only
    ...
    model.conv1 = new_conv1 # Replace layer; weights loaded later
    print(f"Adapted model.conv1 architecture for {input_channels} input channels.")

    # --- Replace the final fully connected layer ---
    num_ftrs = model.fc.in_features
    model.fc = nn.Linear(num_ftrs, num_classes) # Replace layer; weights loaded later
    print(f"Adapted model.fc layer architecture for {num_classes} output classes.")
...
def evaluate_model(config: Dict[str, Any], device: torch.device, num_classes: int, class_names: List[str]):
    ...
    # --- Model ---
    # Create the correct architecture FIRST using the consistent function
    model = adapt_resnet_for_multichannel(
        model_name="resnet50", # Ensure this matches model used in training
        pretrained=config["model"]["pretrained"], # Consistent flag (False for scratch)
        num_classes=num_classes,
        input_channels=config["model"]["input_channels"] # Should be 16 channels
    )

    # Load the saved weights from the SCRATCH training
